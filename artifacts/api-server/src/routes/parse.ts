...
12. Airport Transfer 詳細解析規則：
    ...
    h. 司機姓名、車牌（如「司機 陳彥翔 車牌 RFH-7077」）→ 這是工作人員資訊，放入 notes，絕不放入 passengerName 或 passengerPhone。
    i. 訂單識別碼、授權碼等英數字串（如 AR87260623601O、AB8830）→ 放入 notes。
    j. amount：若原文明確描述「未來將收到款項」（如「下週四會轉帳，費用$1200」），填入金額數字字串（如 "1200"）。
    k. 條件式現金提醒（如「如客戶有給現金，確認是否為小費」）= 不是財務事項，不得填入 amount，amount 保持 null。
    l. price 欄位：只用於既有報價參考顯示，不用來建立財務事項。若 amount 已填，price 保持 null。
