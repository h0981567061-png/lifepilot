...
// Describes how a financial due date was derived from the original message.
// Used so the frontend can recalculate when the event date changes.
interface AIFinancialDateSource {
  // "absolute"         — explicit calendar date ("8/5", "2026/08/05")
  // "relative_to_event"— relative expression ("下週四", "三天後", "課程結束後3天")
  type: "absolute" | "relative_to_event";
  rawText: string;                         // the original date expression
  baseField: "startDate" | "endDate";      // which event date is the reference
}
...
interface AIEvent {
  ...
  financialDateSource: AIFinancialDateSource | null; // 款項日期來源；無款項時填 null
}
...
15. 款項日期來源（financialDateSource 欄位，每個 event 必填）：
    a. 若事項有 amount（即存在關聯款項），必須填寫 financialDateSource：
       ・款項日期為絕對日期（如「8/5」「2026/08/05」「下個月5號」）：
         {"type":"absolute","rawText":"8/5","baseField":"startDate"}
       ・款項日期為相對語意（如「下週四」「三天後」「完工後7天」「課程結束後3天」「隔天」「一週後」）：
         {"type":"relative_to_event","rawText":"下週四","baseField":"startDate"}
         若相對的是事項「結束日期」（如「課程結束後3天」「活動結束隔天收費」）：
         {"type":"relative_to_event","rawText":"課程結束後3天","baseField":"endDate"}
    b. 判斷 absolute vs relative_to_event 的規則：
       ・有明確的月份數字（如「8月5日」「8/5」「下個月5號」）→ absolute
       ・使用相對語意詞（「下週X」「N天後」「隔天」「N週後」「完工/課程/活動/事件後」）→ relative_to_event
    c. 若沒有 amount，financialDateSource = null。
    d. financialDateSource 欄位每個 event 都必填（無款項時填 null）。
...
function validateAIResult(data: unknown): data is AIParseResult {
  ...
  for (const e of d.events) {
    ...
    if (!("financialDateSource" in ev)) {
      (ev as Record<string, unknown>).financialDateSource = null;
    }
  }
  return true;
}
